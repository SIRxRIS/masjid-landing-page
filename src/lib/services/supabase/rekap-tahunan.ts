...
export async function getRekapPemasukanTahunan(
  ...
  try {
    ...
    if (kotakAmalMasjidError) {
      console.error(
        "❌ Error mengambil data kotak amal masjid:",
    ...
    if (kotakAmalJumatError) {
      console.error(
        "❌ Error mengambil data kotak amal jumat:",
        kotakAmalJumatError,
      );
    }
    ...
    if (donasiKhususError) {
      console.error(
        "❌ Error mengambil data donasi khusus:",
        donasiKhususError,
      );
    }
    ...
    console.log(
      `✅ Berhasil mengambil ${rekapData.length} sumber pemasukan untuk tahun ${tahun}`,
    );

    // Log detail untuk debugging
    rekapData.forEach((item) => {
      console.log(
        `📊 ${item.sumber}: Rp ${item.total.toLocaleString("id-ID")}`,
      );
    });
